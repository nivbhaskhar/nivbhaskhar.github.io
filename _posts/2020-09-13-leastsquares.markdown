...
In an experiment, we observe the values of some *features*{: style="color: hotpink"} or *attributes*{: style="color: hotpink"}. We would like to predict the value of a *target*{: style="color: hotpink"} variable based on the observed features. Linear regression tries to guess a linear relationship between the features and the target.
...
Abstracting out this set up, we can assume we are given data points $$x_1, x_2, \ldots x_n$$ where each data point $$x_i$$ has $$k$$ *features*{: style="color: hotpink"} and has target $$y_i$$. That is, each data point $$x_i$$ looks like the row vector 
...
*Introducing a dummy feature for uniformity*{: style="color: hotpink"} : The $$\theta_0$$ stands out without a binding feature $$x_i^0$$, so to make it uniform, we introduce a dummy feature $$x_i^0 = 1$$ for each datapoint $$x_i$$. That is, each data point $$x_i$$ now looks like the row vector 
...
That is, we want a *least-squares solution*{: style="color: hotpink"} of the matrix equation $$X\theta = Y$$

# An analytic approach
{: style="text-align: center; background-color: pink"}


*If you are uncomfortable with multivariable derivatives, skip ahead to the geometric approach!*{: style="color: hotpink"}
...
The matrix equation $$X^TX\theta=X^TY$$ is called the *normal equation*{: style="color: hotpink"} of $$X\theta=Y$$. And from our discussion above, it should be more or less clear that a solution $$\theta$$ of the normal equation gives a least-squares solution of $$X\theta=Y$$.
...
* The least-squares solutions $$\theta$$ of the equation $$X\theta = Y$$ are the same as the solutions of the *normal equation*{:style = "color:hotpink"} $$X^TX\theta = X^TY$$.
